...
1060    rem IF ME% = 0 THEN PRINT "Demo will not be functional!"
1080    rem IF ME% = 0 THEN PRINT CHR$ (7) : END2
...
1240  PRINT "[2] DEBUG (80 COL)"
1250  NORMAL
1260  IF SL% = 2 THEN INVERSE
1270  PRINT "[3] RELEASE (40 COL)"
1280  NORMAL
1290  IF SL% = 3 THEN INVERSE
1300  PRINT "[4] RELEASE (80 COL)"
1310  NORMAL
1320  PRINT : PRINT "[SPC/ARROWS/1-4] to Change"
1330  PRINT "[Enter/ESC] to Confirm"
1340  PRINT : PRINT "Your Choice:";
1350  GET A$
1360  K = ASC(A$)
1370  IF K = 13 THEN 1500
1375  IF K = 27 THEN END
1380  IF K = 32 OR K = 21 OR K = 10 THEN SL% = SL% + 1 : IF SL% > 3 THEN SL% = 0 : GOTO 1170
1390  IF K = 8 OR K = 11 THEN SL% = SL% - 1 : IF SL% < 0 THEN SL% = 3 : GOTO 1170
1400  IF K = 49 THEN SL% = 0 : GOTO 1460
1410  IF K = 50 THEN SL% = 1 : GOTO 1460
1420  IF K = 51 THEN SL% = 2 : GOTO 1460
1430  IF K = 52 THEN SL% = 3 : GOTO 1460
1460  IF SL% = 0 THEN DB% = 1 : CB% = 0
1470  IF SL% = 1 THEN DB% = 1 : CB% = 1
1480  IF SL% = 2 THEN DB% = 0 : CB% = 0
1490  IF SL% = 3 THEN DB% = 0 : CB% = 1
1495  GOTO 1170
...
2000  IF DB% = 1 THEN PRINT "Debug in 80 Col, DHGR, Loading..." : PRINT : PRINT "->TITLE.DHGR (MAIN at $2000)"
